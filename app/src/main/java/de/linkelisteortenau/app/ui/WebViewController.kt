package de.linkelisteortenau.app.ui
...
/**
 * Class for the Brain of WebView Content
 **/
class WebViewController(
     ...
     * else show WebView
     *
     * @param activity as Fragment Activity for check transport network capabilities
     * @param binding for WebView bindings
     * @param url the URL for WebView
     * @param parentLink the URL of the parent category, if back pressed jump to parent link
     **/
    fun run(
        ...
        if (Connection(context).isOnline()) {
            runWebView(activity, binding, url, parentLink)
        } else {
     ...
     * @param binding for WebView bindings
     * @param url the URL for WebView
     * @param parentLink for the current parent Category URL
     **/
    ...
    private fun runWebView(
        ...
        activity.onBackPressedDispatcher.addCallback(activity) {
            ...
            if (webView.canGoBack()) {
                webView.goBack()
            } else if (url != parentLink) {
                // Handle if parent Link exist and back button pressed
                webView.loadUrl(parentLink)
            }
        }.isEnabled
